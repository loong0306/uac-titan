...
public class SwaggerConfig {
    ...
    public Docket reservationApi() {
        return new Docket(DocumentationType.SWAGGER_2).
                apiInfo(apiInfo())
                .select()
                .apis(RequestHandlerSelectors.basePackage("me.uac.restful"))
                .paths(PathSelectors.any())
                .build();
    }

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("用户中心 | UAC-TITAN")
		        .description("The API for UAC-TITAN.").version("1.0").build();
	}
